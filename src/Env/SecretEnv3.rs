...
impl SecretEnv3 {
    ...
    pub fn p(&self, s: usize, a: usize, s_p: usize, r: usize) -> f32 {
        ...
        unsafe {
            secret_env_3_transition_probability(s, a, s_p, r)
        }
    }

    pub fn num_states(&self) -> i32 {
        let secret_env_3_num_states: libloading::Symbol<unsafe extern fn() -> usize> =
            unsafe {LIB.get(b"secret_env_3_num_states").expect("Failed to load function `secret_env_3_num_states`")};
        unsafe {
            secret_env_3_num_states() as i32
        }
    }
    ...
    pub fn agent_pos(&self) -> i32{
        ...
        unsafe {
            secret_env_3_state_id(self.env) as i32
        }
    }

    pub fn num_actions(&self) -> i32 {
        let secret_env_3_num_actions: libloading::Symbol<unsafe extern fn() -> usize> =
            unsafe{LIB.get(b"secret_env_3_num_actions").expect("Failed to load function `secret_env_3_num_actions`")};
        unsafe {
            secret_env_3_num_actions() as i32
        }
    }

    pub fn A(&self) -> Vec<usize> {
        let secret_env_3_available_actions: libloading::Symbol<unsafe extern fn(*const c_void) -> *const usize> =
            unsafe { LIB.get(b"secret_env_3_available_actions").expect("Failed to load function `secret_env_3_available_actions`") };
        unsafe {
    ...
    pub fn available_actions_len(&self) -> i32 {
        let secret_env_3_available_actions_len: libloading::Symbol<unsafe extern fn(*const c_void) -> usize> =
            unsafe{LIB.get(b"secret_env_3_available_actions_len").expect("Failed to load function `secret_env_3_available_actions_len`")};
        unsafe {
            secret_env_3_available_actions_len(self.env) as i32
        }
    }
    ...
    pub fn is_forbidden(&self, action: i32) -> bool {
        let secret_env_3_is_forbidden: libloading::Symbol<unsafe extern fn(*const c_void, usize) -> bool> =
            unsafe{LIB.get(b"secret_env_3_is_forbidden").expect("Failed to load function `secret_env_3_is_forbidden`")};
        unsafe {
            secret_env_3_is_forbidden(self.env, action as usize)
        }
    }
    ...
    pub fn num_rewards(&self) -> i32 {
        let secret_env_3_num_rewards: libloading::Symbol<unsafe extern fn() -> usize> =
            unsafe{LIB.get(b"secret_env_3_num_rewards").expect("Failed to load function `secret_env_3_num_rewards`")};
        unsafe {
            secret_env_3_num_rewards() as i32
        }
    }

    pub fn R(&self, pos: usize) -> f32 {
        let secret_env_3_reward: libloading::Symbol<unsafe extern fn(usize) -> f32> =
            unsafe{LIB.get(b"secret_env_3_reward").expect("Failed to load function `secret_env_3_reward`")};
        unsafe {
            secret_env_3_reward(pos)
        }
    }

    pub fn is_game_over(&self) -> bool {
        let secret_env_3_is_game_over: libloading::Symbol<unsafe extern fn(*const c_void) -> bool> =
            unsafe{LIB.get(b"secret_env_3_is_game_over").expect("Failed to load function `secret_env_3_is_game_over`")};
        unsafe {
    ...
    pub fn from_random_state(&mut self) -> *mut c_void {
        let secret_env_3_from_random_state: libloading::Symbol<unsafe extern fn() -> *mut c_void> =
            unsafe{LIB.get(b"secret_env_3_from_random_state").expect("Failed to load function `secret_env_3_from_random_state`")};
        unsafe {
            secret_env_3_from_random_state()
        }
    }

    pub fn score(&self) -> f32 {
        let secret_env_3_score: libloading::Symbol<unsafe extern fn(*const c_void) -> f32> =
            unsafe{LIB.get(b"secret_env_3_score").expect("Failed to load function `secret_env_3_score`")};
        unsafe {
    ...
    pub fn step(&mut self, action: i32) {
        let secret_env_3_step: libloading::Symbol<unsafe extern fn(*mut c_void, usize)> =
            unsafe{LIB.get(b"secret_env_3_step").expect("Failed to load function `secret_env_3_step`")};
        unsafe{
            secret_env_3_step(self.env, action as usize);
        }
    }
    pub fn reset(&self) {
        let secret_env_3_reset: libloading::Symbol<unsafe extern fn(*const c_void) -> usize> =
            unsafe{LIB.get(b"secret_env_3_reset").expect("Failed to load function `secret_env_3_reset`")};
        unsafe {
