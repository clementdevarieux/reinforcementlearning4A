...
impl SecretEnv1 {
    ...
    pub fn p(&self, s: usize, a: usize, s_p: usize, r: usize) -> f32 {
        ...
        unsafe {
            secret_env_1_transition_probability(s, a, s_p, r)
        }
    }

    pub fn num_states(&self) -> i32 {
        let secret_env_1_num_states: libloading::Symbol<unsafe extern fn() -> usize> =
            unsafe {LIB.get(b"secret_env_1_num_states").expect("Failed to load function `secret_env_1_num_states`")};
        unsafe {
            secret_env_1_num_states() as i32
        }
    }
    ...
    pub fn agent_pos(&self) -> i32{
        ...
        unsafe {
            secret_env_1_state_id(self.env) as i32
        }
    }

    pub fn num_actions(&self) -> i32 {
        let secret_env_1_num_actions: libloading::Symbol<unsafe extern fn() -> usize> =
            unsafe{LIB.get(b"secret_env_1_num_actions").expect("Failed to load function `secret_env_1_num_actions`")};
        unsafe {
            secret_env_1_num_actions() as i32
        }
    }

    pub fn A(&self) -> Vec<usize> {
        let secret_env_1_available_actions: libloading::Symbol<unsafe extern fn(*const c_void) -> *const usize> =
            unsafe { LIB.get(b"secret_env_1_available_actions").expect("Failed to load function `secret_env_1_available_actions`") };
        unsafe {
            let actions_ptr = secret_env_1_available_actions(self.env);
    ...
    pub fn available_actions_len(&self) -> i32 {
        let secret_env_1_available_actions_len: libloading::Symbol<unsafe extern fn(*const c_void) -> usize> =
            unsafe{LIB.get(b"secret_env_1_available_actions_len").expect("Failed to load function `secret_env_1_available_actions_len`")};
        unsafe {
            secret_env_1_available_actions_len(self.env) as i32
        }
    }
    ...
    pub fn is_forbidden(&self, action: i32) -> bool {
        let secret_env_1_is_forbidden: libloading::Symbol<unsafe extern fn(*const c_void, usize) -> bool> =
            unsafe{LIB.get(b"secret_env_1_is_forbidden").expect("Failed to load function `secret_env_1_is_forbidden`")};
        unsafe {
            secret_env_1_is_forbidden(self.env, action as usize)
        }
    }
    ...
    pub fn num_rewards(&self) -> i32 {
        let secret_env_1_num_rewards: libloading::Symbol<unsafe extern fn() -> usize> =
            unsafe{LIB.get(b"secret_env_1_num_rewards").expect("Failed to load function `secret_env_1_num_rewards`")};
        unsafe {
            secret_env_1_num_rewards() as i32
        }
    }

    pub fn R(&self, pos: usize) -> f32 {
        let secret_env_1_reward: libloading::Symbol<unsafe extern fn(usize) -> f32> =
            unsafe{LIB.get(b"secret_env_1_reward").expect("Failed to load function `secret_env_1_reward`")};
        unsafe {
            secret_env_1_reward(pos)
        }
    }

    pub fn is_game_over(&self) -> bool {
        let secret_env_1_is_game_over: libloading::Symbol<unsafe extern fn(*const c_void) -> bool> =
            unsafe{LIB.get(b"secret_env_1_is_game_over").expect("Failed to load function `secret_env_1_is_game_over`")};
        unsafe {
            secret_env_1_is_game_over(self.env)
        }
    }

    pub fn display(&self) {
        let secret_env_1_display: libloading::Symbol<unsafe extern fn(*const c_void)> =
            unsafe{LIB.get(b"secret_env_1_display").expect("Failed to load function `secret_env_1_display`")};
        unsafe {
    ...
    pub fn from_random_state(&mut self) -> *mut c_void {
        let secret_env_1_from_random_state: libloading::Symbol<unsafe extern fn() -> *mut c_void> =
            unsafe{LIB.get(b"secret_env_1_from_random_state").expect("Failed to load function `secret_env_1_from_random_state`")};
        unsafe {
            secret_env_1_from_random_state()
        }
    }

    pub fn score(&self) -> f32 {
        let secret_env_1_score: libloading::Symbol<unsafe extern fn(*const c_void) -> f32> =
            unsafe{LIB.get(b"secret_env_1_score").expect("Failed to load function `secret_env_1_score`")};
        unsafe {
    ...
    pub fn step(&mut self, action: i32) {
        let secret_env_1_step: libloading::Symbol<unsafe extern fn(*mut c_void, usize)> =
            unsafe{LIB.get(b"secret_env_1_step").expect("Failed to load function `secret_env_1_step`")};
        unsafe{
            secret_env_1_step(self.env, action as usize);
        }
    }
    pub fn reset(&self) {
        let secret_env_1_reset: libloading::Symbol<unsafe extern fn(*const c_void) -> usize> =
            unsafe{LIB.get(b"secret_env_1_reset").expect("Failed to load function `secret_env_1_reset`")};
        unsafe {
            secret_env_1_reset(self.env);
        }
    }
    ...
    pub fn state_id(&self) -> usize{
        let secret_env_1_state_id: libloading::Symbol<unsafe extern fn(*const c_void) -> usize> =
            unsafe{LIB.get(b"secret_env_1_state_id").expect("Failed to load function `secret_env_1_state_id`")};
        unsafe {
            secret_env_1_state_id(self.env)
        }
    }
    ...
    pub fn monte_carlo_fv_on_policy(&mut self,
        ...
        let mut Pi: HashMap<i32, HashMap<i32, f32>> = HashMap::new();
        ...
        for _ in 0..nb_iter {
            ...
            while steps_count < max_steps && !self.is_game_over() {
                ...
                if self.is_game_over(){
                    ...
                    let random_value: f32 = rng.gen();
                    if random_value < epsilon {
                        a_p = *self.autorized_actions().choose(&mut rng).unwrap()
                    } else {
                        let mut best_a_p: Option<i32> = None;
                        let mut best_a_score_p: Option<f32> = None;

                        for a_p in self.autorized_actions() {
